...
This optimization comes without the source because it changes the contract between the caller and ParsedStr class.

We can avoid making a copy of the passed string if we can take ownership of the string. This requires coordinations between ParsedStr and its callers (the caller allocates the string but ParsedStr frees it).
