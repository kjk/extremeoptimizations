...
In naive version we make N*2 allocations to store N name/value pairs. Most of those strings are short, so we're probably badly hit by @malloc()@'s overhead. Furthermore, @malloc()@ is relatively slow, so doing less of it can only be good.
...
Do we need the offsets at all? We know that strings are laid out in memory in a specific order and if we're willing to sacrifice a little bit of speed, we don't have to store the offsets. Let's just find n-th string from the beginning.
...
If there are just few strings and @name()@ and @value()@ are called only a couple of times then it doesn't matter.
...
This trick can be used in combination with other optimizations since it only deals with how the string data is being allocated.

h2. Comparing different approaches.

TODO: stats on executable size.

h2. Conclusions.

STL code is arguably the cleanest but you pay a huge price in overhead and inflexibility. TOD: stats on code size.
...
Some of the above techniques can't be expressed in STL and some of them can but you'll no longer get the advantage of small, simple code, which eliminates pretty much the only thing STL has going for itself.
